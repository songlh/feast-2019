...
%and a FSM loop conditionally update its state variable in each iteration.  
...
\commentlh{remember to mention a distinct input is processed in each iteration 
and remember to discuss how to identify variables or memory locations holding inputs}
...
\commentlh{remember to discuss a transition function is executed in one iteration}
...
\commentlh{\texttt{state} can be any value when the loop finishes its execution}
